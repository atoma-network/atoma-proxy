...
pub async fn handle_node_key_rotation_event(
    ...
    #[cfg(feature = "confidential")]
    let is_valid = match event.tee_provider {
        ...
        atoma_confidential::types::TEEProvider::Snp => {
            utils::snp::verify_snp_attestation(&tee_remote_attestation_bytes, &new_public_key)
                .await
                .is_ok()
        },
        atoma_confidential::types::TEEProvider::Arm => {
            todo!()
        }
    };
...
mod utils {
    ...
    pub(crate) mod snp {
        ...
        pub(crate) async fn verify_snp_attestation(
            tee_remote_attestation_bytes: &[u8],
            new_public_key: &[u8],
        ) -> Result<()> {
            let snp_attestation: SNPAttestationReport = SNPAttestationReport::from_bytes(tee_remote_attestation_bytes)
                .map_err(|e| AtomaStateManagerError::FailedToParseQuote(format!("{e:?}")))?;

            // First check if the report data matches the newly committed public key
            if snp_attestation.report.report_data != new_public_key {
                return Err(AtomaStateManagerError::FailedToVerifyQuote(
                    "Report data does not match new public key".to_string(),
                ));
            }

            // Verify the SNPAttestationReport using its implementation of the Verifiable trait
            match snp_attestation.verify() {
                Ok(_) => (),
                Err(e) => {
                    return Err(AtomaStateManagerError::FailedToVerifyQuote(format!("{e:?}")));
                }
            }
    ...
    //     pub(crate) async fn verify_arm_attestation(
    //         tee_remote_attestation_bytes: &[u8],
    //         new_public_key: &[u8],
    //     ) -> Result<()> {
    //         todo!()
